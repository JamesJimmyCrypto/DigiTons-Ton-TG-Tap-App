...
const myMonstrixSlice = createSlice({
  ...
  initialState: {
    myMonstrix: [
      allMonstrix.find((elem) => elem.name === "Doge"),
      allMonstrix.find((elem) => elem.name === "Pepe"),
      allMonstrix.find((elem) => elem.name === "Milady"),
    ],
  ...
  reducers: {
    levelUp(state, action) {
      if (action.payload.id !== 0) {
        let myMonster = state.myMonstrix.find(
    ...
    addNewMonster(state, action) {
      let newMonster = allMonstrix.find(
        (elem) => elem.id === action.payload.id
      );
